...
vector<long double> dc_shift_normalize_vac(vector<long double> data)//dc shift then voice activity detection and then normalize
{
	...
	long double a = 0, b = 0, counta = 0, countb = 0, dc_shift_bound = 1000, threshold, first_high, last_high, max_v = INT_MIN, min_v = INT_MAX, amplitude = 5000,dc_shift_value=0,count=0;
	...
	b = a;
	threshold =1000*a;//defining threshold with the help of average ambient sound for positive and negative sides
...
vector<vector <long double> > get_input_cis(string filepath, long double num_samples, long double p)//get the input from the file and calculate the ci's which is the input to the program
{
	long double item,G2;
...
long double tokura_distance(vector<vector <long double> > a, vector<vector <long double> > b,vector <long double> weights)//finding tokhura's distance
{
...
int find_row_with_min_dist(vector < vector <long double> > input)//findingin the index with the least distance where index tells the vowels number
{
	int row = 0;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (long long int i = 0; i < files_each_vowel; i++)
	{
		for (long long int j = 0; j < vowels.size(); j++)
		{
			...
			all_data = data;
			all_data = dc_shift_normalize_vac(data);//does dc shift, normalize the data and do voice activity detecting
	...
	recog_index=find_row_with_min_dist(all_distances);//finding the vowel index;
	cout << "Recognized vowel is " << vowels[recog_index] << endl;
